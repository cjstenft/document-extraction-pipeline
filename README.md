...
actually on the page. Ground truth was manually spot-checked during
development, which is how a rendering bug was caught where unescaped `&`
characters in vendor names were showing up as literal `&amp;` on the page.
...
Field-level accuracy:  99.1% (743/750 fields)
Throughput:            10.2 documents/minute
Test set:              35 documents (12 purchase orders, 12 packing slips, 11 bills of lading)
```

A repeat run of this exact code scored 99.5% rather than 99.1% — see the
run-to-run variance bullet in Limitations before treating either number as
precise.
...
Evaluation Method above) before this number was produced. Read 99.1% as
...
| Model | Field-level accuracy | Throughput | Cost/document |
|---|---|---|---|
| `claude-opus-5` (effort=high, API default) | 99.1% (743/750) | 10.2 docs/min | $0.0247 |
| `claude-opus-5` (effort=low) | 99.3% (745/750) | 9.5 docs/min | $0.0242 |
| `claude-sonnet-5` | 99.6% (747/750) | 11.6 docs/min | $0.0150 |
...
over-explanation — just at different rates (7 mismatches for Opus, 3 for
Sonnet, out of 135 line items each). With only 35 documents and no
temperature control on either model, a few-mismatch swing between runs is
within normal variance (see Limitations), so this isn't strong evidence
that Sonnet is categorically more accurate than Opus. It is good evidence
that Opus's extra cost isn't buying a measurable accuracy improvement on
this task — which is the more actionable finding of the two. Based on this,
`extract.py`'s default model was changed from `claude-opus-5` to
`claude-sonnet-5` (see Approach) — pass `--model claude-opus-5` to
reproduce the numbers in this table's first row.

Effort made effectively no difference either, and the reason is more
specific than "the task is too constrained": thinking is fully disabled
regardless of effort, not just formally requested to be.
...
full 35-document set (405 vs 386 tokens/document, high vs low) and cost by
only ~2% ($0.0247 vs $0.0242). `effort=low` scored 99.3% (5 mismatches)
against the default's 99.1% (7 mismatches) — a smaller swing than the
run-to-run variance already observed, and if anything in the opposite
direction from what more effort would predict, consistent with effort
doing essentially nothing here rather than doing something and losing.
...
targeted field *worse* — 90.4% (122/135, 13 mismatches) versus the original
94.8% (128/135, 7 mismatches) — and changed the shape of the failure
...
- **The test set is entirely synthetic**, generated by this same pipeline's
  ...
  99.1% on a clean synthetic set should not be read as an estimate of
...
- **35 documents is a small sample, and this measurably fluctuates.**
  ...
  code changes) on a different day moved the headline figure from 99.5% to
  99.1% — a swing of three additional mismatches, all the same failure mode
  ...
  to make a precise accuracy claim with tight confidence bounds.
...
**`line_items.description` was the only field type with any errors: 94.8%
accuracy (128/135 correct, 7 mismatches).** All seven mismatches share the
...
|---|---|
| `Safety Goggles, Case of 12 (box)` | `Safety Goggles, Case of 12` |
